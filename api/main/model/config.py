...
ENV = 'DEV' if socket.gethostname() == 'LAPTOP-F3U1J437' else 'PROD'

FNT_URL = 'http://oss-fnt-app-1'
FNT_API_URLS = {
    'login': FNT_URL +'/axis/api/rest/businessGateway/login'} if ENV == "DEV" else {'login': 'http://iec-inventory:8080/axis/api/rest/businessGateway/login'}

FNT_DB_config = {
    'fntDbUser': 'command',
    'fntDbPassword': 'command',
    'fntDsn': '(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=oss-fnt-app-1)(PORT=1521))(CONNECT_DATA=(SID=fntdb)))',
    'fntDbMaxConn': 10,
    'manId': "1001",
...
def get_db_con():
    ...
    if socket.gethostname() == 'LAPTOP-F3U1J437':
        con = mysql.connector.connect(port='8080',
                                    host='localhost',
                                  user='root',
                                  password='root',
                                  database="relay_task_manager", auth_plugin='mysql_native_password') if ENV == 'DEV' else mysql.connector.connect(host='localhost',
                                                                                                                                                    user='root',
...
QUERIES = {
    ...
    'UPDATE_TASK': """UPDATE task SET title =%(title)s, description = %(description)s, type_id =%(type_id)s, domain_id=%(domain_id)s, father_id=%(father_id)s, status_id=%(status_id)s, urgency_id =%(urgency_id)s, priority =%(priority)s, crew =%(crew)s, start=%(start)s,end=%(end)s, deadline=%(deadline)s,fault_data=%(fault_data)s,file_required =%(file_required)s, files_url=%(files_url)s, plannings=%(plannings)s, building=%(building)s, equipment=%(equipment)s,check_list=%(check_list)s, background=%(background)s, is_reschedule=%(is_reschedule)s where id =%(id)s""",
    'CREATE_TASK': """INSERT INTO task (title, description, type_id, domain_id, father_id, status_id, urgency_id, priority, crew, files_url, plannings, building, equipment, check_list, background, is_template, start, end, deadline, creator_id, fault_data, file_required) VALUES(%(title)s, %(description)s, %(type_id)s, %(domain_id)s, %(father_id)s, %(status_id)s, %(urgency_id)s, %(priority)s, %(crew)s, %(files_url)s, %(plannings)s, %(building)s, %(equipment)s, %(check_list)s, %(background)s, %(is_template)s, %(start)s, %(end)s, %(deadline)s, %(creator_id)s, %(fault_data)s, %(file_required)s)""" if ENV == 'DEV' else """INSERT INTO task (title, description, type_id, domain_id, father_id, status_id, urgency_id, priority, crew, files_url, plannings, building, equipment, check_list, background, is_template, start, end, deadline, creator_id, fault_data,file_required, creation_time, modify_time) VALUES(%(title)s, %(description)s, %(type_id)s, %(domain_id)s, %(father_id)s, %(status_id)s, %(urgency_id)s, %(priority)s, %(crew)s, %(files_url)s, %(plannings)s, %(building) %(equipment)s, %(check_list)s, %(background)s, %(is_template)s, %(start)s, %(end)s, %(deadline)s, %(creator_id)s, %(fault_data)s, %(file_required)s, null, null)""",
    ...
    'UPDATE_FATHER_TASK': """UPDATE father_task SET title =%(title)s, description =%(description)s, manager =%(manager)s, complete_by_order=%(complete_by_order)s, locked =%(locked)s, deadline =%(deadline)s, completed_date=%(completed_date)s, creator_id=%(creator_id)s where id =%(id)s""",
    'CREATE_FATHER_TASK': """INSERT INTO father_task (title, description, manager, complete_by_order, locked, deadline, creator_id, is_template) VALUES(%(title)s, %(description)s, %(manager)s, %(complete_by_order)s, %(locked)s, %(deadline)s, %(creator_id)s, %(is_template)s)""" if ENV == 'DEV' else """INSERT INTO father_task (title, description, manager, complete_by_order, locked, deadline, creator_id, is_template, creation_time, modify_time) VALUES(%(title)s, %(description)s, %(manager)s, %(complete_by_order)s, %(locked)s, %(deadline)s, %(creator_id)s, %(is_template)s, null, null)""",
    ...
    'GET_TASK_TO_RESCHEDULE': "SELECT * FROM task WHERE background = false and end < DATE_SUB(DATE(NOW()), INTERVAL 3 DAY) and status_id!=3 and is_template = false",
...
FNT_QUERIES = {
    ...
    'GET_BUILDINGS_BY_SEARCH': """SELECT INTERNAL_ID AS internal_id ,NAME AS  name ,C_X AS lng ,C_Y AS lat FROM META_BUILDING mb WHERE C_X IS NOT NULL AND C_Y IS NOT NULL AND upper(NAME ) like '%{}%'"""
...
class FNT_QUERIES_NAMES:
    ...
    GET_BUILDINGS_BY_SEARCH = 'GET_BUILDINGS_BY_SEARCH'
